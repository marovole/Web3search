...
## 5. 测试框架修复
- [x] 5.1 更新所有Playwright测试选择器匹配生产DOM - 使用更宽松的选择器
- [x] 5.2 修复API集成测试使用前端代理路径 - 区分开发和生产环境
- [x] 5.3 在 tests/e2e/chat.spec.ts 中添加等待逻辑 - 增加重试和超时
- [x] 5.4 增加测试错误诊断和截图功能 - 监听控制台和网络请求
- [x] 5.5 配置测试超时设置（历史页面导航120秒） - 已配置各种超时

## 6. 部署验证增强
- [x] 6.1 创建部署后烟雾测试脚本（smoke-test.js） - 已创建并测试
- [x] 6.2 测试关键API端点可达性（/api/health, /api/v1/chat/quick） - 已实现
- [x] 6.3 在CI/CD中添加测试验证步骤 - 创建 smoke-tests.yml workflow
- [ ] 6.4 配置测试失败触发部署回滚 - 需要部署平台支持
- [x] 6.5 集成测试报告生成和通知 - GitHub Issue 自动创建
...
## 8. 文档更新
- [x] 8.1 更新部署文档，说明生产环境API配置 - 创建 DEPLOYMENT.md
- [x] 8.2 记录Cloudflare Pages _redirects 规则说明 - 包含在部署文档中
- [x] 8.3 更新测试文档，说明选择器策略 - 代码注释中已说明
- [x] 8.4 创建故障排除指南（404错误、API调用失败） - 创建 TROUBLESHOOTING.md
- [x] 8.5 更新README中的生产环境访问信息 - 文档已完善
