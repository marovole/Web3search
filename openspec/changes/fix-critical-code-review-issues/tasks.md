...
- [ ] 1.1.1 Update `workers-api/src/routes/chat.ts:215` from `chat.post()` to `chat.get()`
- [ ] 1.1.2 Replace `c.req.json()` with query parameter extraction (`c.req.query()`)
- [ ] 1.1.3 Add input validation for query parameters (required: `query`, optional: `conversation_id`)
- [ ] 1.1.4 Add query length validation (max 2000 chars) with 414 URI Too Long response
- [ ] 1.1.5 Update rate limit middleware key to work with GET requests
- [ ] 1.1.6 Verify SSE stream initialization still works correctly
...
### 1.4 Documentation
- [ ] 1.4.1 Update API documentation to reflect GET method
- [ ] 1.4.2 Document query parameter schema
- [ ] 1.4.3 Document query length limitation (2000 chars)
...
- [ ] 2.1.1 Edit `workers-api/src/lib/telemetry.ts:181` to remove `responseBody` field
- [ ] 2.1.2 Add `responseMetadata` object with safe fields:
  - `bodyLength: number`
  - `contentType: string | null`
  - `hasError: boolean`
- [ ] 2.1.3 Ensure all other telemetry fields remain intact
- [ ] 2.1.4 Verify token counting and billing metrics still work
...
- [ ] 3.1.1 Verify if `backend/app/main.py` exists
- [ ] 3.1.2 If not, create `backend/app/main.py` with FastAPI app initialization
- [ ] 3.1.3 Import existing middleware classes:
  - `RequestSizeLimiterMiddleware`
  - `SQLInjectionProtectionMiddleware`
- [ ] 3.1.4 Register middleware with `app.add_middleware()` in correct order
- [ ] 3.1.5 Import and register existing route modules (analytics, reports)
- [ ] 3.1.6 Add health check endpoint (`/health`)

### 3.2 Middleware Configuration
- [ ] 3.2.1 Configure request size limit (recommend 10MB max)
- [ ] 3.2.2 Configure SQL injection pattern detection
- [ ] 3.2.3 Add CORS middleware with appropriate origins
- [ ] 3.2.4 Verify middleware execution order (size → injection → CORS)
...
- [ ] 4.1.1 Install Vitest and dependencies (`pnpm add -D vitest @vitest/ui`)
- [ ] 4.1.2 Create `workers-api/vitest.config.ts` with coverage thresholds (60%)
- [ ] 4.1.3 Add test scripts to `workers-api/package.json`:
  - `test`: run tests
...
- [ ] 5.1.1 Create `workers-api/src/lib/conversation.ts`
- [ ] 5.1.2 Extract `ensureConversationExists` function from `chat.ts:291`
- [ ] 5.1.3 Extract `persistMessage` function from `chat.ts`
- [ ] 5.1.4 Extract any other duplicated conversation helpers
- [ ] 5.1.5 Add TypeScript type definitions for function signatures
- [ ] 5.1.6 Add JSDoc comments for each exported function
...
- [ ] 5.3.1 Update `workers-api/src/routes/chat.ts` to import from `lib/conversation`
- [ ] 5.3.2 Remove duplicated implementation from `chat.ts`
- [ ] 5.3.3 Run tests to verify no regressions
- [ ] 5.3.4 Update `workers-api/src/routes/chat-v2.ts` to import from `lib/conversation`
- [ ] 5.3.5 Remove duplicated implementation from `chat-v2.ts`
- [ ] 5.3.6 Run tests to verify no regressions
- [ ] 5.3.7 Update `workers-api/src/routes/deep-research.ts` to import from `lib/conversation`
- [ ] 5.3.8 Remove duplicated implementation from `deep-research.ts`
- [ ] 5.3.9 Run tests to verify no regressions
...
- [ ] 6.1.1 Update API documentation with new SSE endpoint contract
